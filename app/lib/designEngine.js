/**
 * MailForge Design Engine v2.2
 * Premium block-based email design system.
 * Includes 3 layout variants that rotate randomly on each generation.
 *
 * New in v2.2:
 * - Uses brand's actual website fonts (displayFont / bodyFont from scraper)
 * - Falls back to tone-based font pairings only when no site fonts are detected
 * - Larger, bolder hero typography for premium feel
 * - Improved section padding throughout
 */
...
export function getDesignSystem(brandTone) {
  const base = {
    ...
    button: { paddingV: 16, paddingH: 44 },
    maxWidth: 600,
  }
  const toneTokens = {
    'Luxury & refined': {
    ...
    'Bold & direct': {
      ...
      button: { paddingV: 18, paddingH: 44 },
      buttonRadius: '2px',
      labelSpacing: '4px',
    ...
    'Playful & fun': {
      ...
      button: { paddingV: 16, paddingH: 36 },
      buttonRadius: '30px',
      labelSpacing: '2px',
      sectionDivider: false,
      heroStyle: 'playful',
...
// ─── FONT SYSTEM ──────────────────────────────────────────────────────────────
// Tone-based fallback pairings — used only when scraper finds no site fonts
...
// Builds a Google Fonts import URL from two font names
...
// Main font resolver — uses brand's actual site fonts first, falls back to tone pairings
export function getFontPairing(tone, brandData = {}) {
  const fallback = TONE_FONT_FALLBACKS[tone] || TONE_FONT_FALLBACKS['Warm & friendly']

  // If the AI detected and returned actual site fonts, use those
  ...
  // If only one font detected, use it for display and fall back for body
...
<tr>
  <td bgcolor="${primaryColor}" style="padding:44px 52px 52px;text-align:left;">
    <h1 style="margin:0 0 22px;font-family:${df};font-size:${h1}px;font-weight:700;line-height:1.1;letter-spacing:-0.5px;color:${primaryTextColor};max-width:440px;">${copy.hero_headline || ''}</h1>
    <a href="#" style="display:inline-block;background:transparent;border:1px solid ${primaryTextColor};color:${primaryTextColor};font-family:${bf};font-size:10px;font-weight:600;letter-spacing:3px;text-transform:uppercase;text-decoration:none;padding:${bpv}px ${bph}px;border-radius:${br};">${copy.cta_button || 'DISCOVER'}</a>
  </td>
</tr>`
}
...
<tr><td bgcolor="${primaryColor}" style="padding:80px 56px;text-align:center;">
  <h1 style="margin:0 0 24px;font-family:${df};font-size:${h1}px;font-weight:700;line-height:1.1;letter-spacing:-0.5px;color:${primaryTextColor};">${copy.hero_headline || ''}</h1>
  <p style="margin:0 auto 40px;max-width:440px;font-family:${bf};font-size:${bodySize}px;font-weight:400;line-height:1.8;color:${primaryTextColor};opacity:0.75;">${copy.hero_subline || ''}</p>
  <a href="#" style="display:inline-block;background:${accentColor};color:${accentTextColor};font-family:${bf};font-size:11px;font-weight:600;letter-spacing:${ds.labelSpacing || '4px'};text-transform:uppercase;text-decoration:none;padding:${bpv}px ${bph}px;border-radius:${br};">${copy.cta_button || 'SHOP NOW'}</a>
</td></tr>`
}
...
<tr><td bgcolor="${primaryColor}" style="padding:80px 56px;text-align:center;">
  ...
  <p style="margin:0 auto 40px;max-width:460px;font-family:${bf};font-size:${bodySize}px;font-weight:400;line-height:1.7;color:${primaryTextColor};opacity:0.85;">${copy.hero_subline || ''}</p>
...
export function heroBlock_warm({ copy, primaryColor, primaryTextColor, accentColor, accentTextColor, df, bf, ds }) {
  ...
  return `
<tr><td bgcolor="${primaryColor}" style="padding:80px 56px;text-align:center;">
  <h1 style="margin:0 0 24px;font-family:${df};font-size:${h1}px;font-weight:700;line-height:1.15;color:${primaryTextColor};">${copy.hero_headline || ''}</h1>
  <p style="margin:0 auto 40px;max-width:460px;font-family:${bf};font-size:${bodySize}px;font-weight:400;line-height:1.8;color:${primaryTextColor};opacity:0.88;">${copy.hero_subline || ''}</p>
...
<tr><td bgcolor="${bgColor}" style="padding:${ds.spacing.xl}px ${ds.spacing.xl}px;">
  ...
  <h2 style="margin:0 0 ${ds.spacing.md}px;font-family:${df};font-size:${h2}px;font-weight:700;line-height:1.2;color:#1a1a1a;">${copy.story_headline || ''}</h2>
  <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="40" style="margin-bottom:${ds.spacing.md}px;"><tr><td style="height:2px;background:${accentForLightBg};font-size:0;line-height:0;">&nbsp;</td></tr></table>
  <p style="margin:0 0 ${ds.spacing.sm}px;font-family:${bf};font-size:${bodySize}px;font-weight:400;line-height:1.9;color:#5a504a;">${copy.story_p1 || ''}</p>
  ${pullQuoteHtml}
  <p style="margin:0 0 ${copy.story_p3 ? ds.spacing.sm + 'px' : '0'};font-family:${bf};font-size:${bodySize}px;font-weight:400;line-height:1.9;color:#5a504a;">${copy.story_p2 || ''}</p>
  ${copy.story_p3 ? `<p style="margin:0;font-family:${bf};font-size:${bodySize}px;font-weight:400;line-height:1.9;color:#5a504a;">${copy.story_p3}</p>` : ''}
</td></tr>`
}
...
<tr><td bgcolor="${bgColor}" style="padding:${ds.spacing.xl}px ${ds.spacing.lg}px;">
  ...
  <p style="margin:0 0 ${ds.spacing.sm}px;font-family:${bf};font-size:${bodySize}px;line-height:1.85;color:#555555;">${copy.story_p1 || ''}</p>
  ${pullQuoteHtml}
  <p style="margin:0 0 ${copy.story_p3 ? ds.spacing.sm + 'px' : '0'};font-family:${bf};font-size:${bodySize}px;line-height:1.85;color:#555555;">${copy.story_p2 || ''}</p>
  ${copy.story_p3 ? `<p style="margin:0;font-family:${bf};font-size:${bodySize}px;line-height:1.85;color:#555555;">${copy.story_p3}</p>` : ''}
</td></tr>`
}
...
export function productBlock_grid3({ products, copy, bgColor, accentForLightBg, df, bf, ds, ctaLabel = 'SHOP NOW', productCardBg = '#f0ede8' }) {
  ...
  const cells = products.map(p => `
    <td width="${colWidth}" style="padding:${ds.spacing.xs}px;text-align:center;vertical-align:top;">
      ...
      <p style="margin:${ds.spacing.sm}px 0 6px;font-family:${bf};font-size:14px;font-weight:600;color:#111111;letter-spacing:0.3px;">${p.name}</p>
...
<tr><td bgcolor="${bgColor}" style="padding:${ds.spacing.xl}px ${ds.spacing.md}px;">
  ...
  <h2 style="margin:0 0 ${ds.spacing.lg}px;font-family:${df};font-size:${ds.typography.h3}px;font-weight:700;color:#111111;text-align:center;">${copy.product_headline || ''}</h2>
  ...
  <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%">
    <tr>
      ...
      <td style="vertical-align:middle;">
        ...
        <h2 style="margin:0 0 ${ds.spacing.sm}px;font-family:${df};font-size:${ds.typography.h3}px;font-weight:700;line-height:1.3;color:#111111;">${p.name}</h2>
        <p style="margin:0 0 ${ds.spacing.md}px;font-family:${bf};font-size:14px;line-height:1.75;color:#777777;">${copy.story_p1 || ''}</p>
        ...
        <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%">
          <tr>
            ...
            <td style="padding-left:${ds.spacing.sm}px;">
              <p style="margin:0 0 4px;font-family:${bf};font-size:14px;font-weight:600;color:#111111;">${p.name}</p>
            </td>
...
<tr><td bgcolor="#f5f3ef" style="padding:${ds.spacing.xl}px ${ds.spacing.xl}px;text-align:center;">
  <p style="margin:0 0 6px;font-family:${df};font-size:52px;font-weight:700;line-height:0.7;color:${accentColor};">"</p>
  <p style="margin:0 auto ${ds.spacing.md}px;max-width:460px;font-family:${df};font-size:24px;font-weight:400;font-style:italic;line-height:1.65;color:#3a3028;">${quote}</p>
  <table role="presentation" cellpadding="0" cellspacing="0" border="0" align="center" style="margin-bottom:${ds.spacing.sm}px;">
    <tr><td style="height:2px;width:40px;background:${accentColor};font-size:0;line-height:0;">&nbsp;</td></tr>
  </table>
  <p style="margin:0 0 6px;font-family:${bf};font-size:10px;font-weight:600;letter-spacing:4px;text-transform:uppercase;color:#9a8c7e;">— ${attribution}, Verified Customer</p>
...
<tr><td bgcolor="#111111" style="padding:${ds.spacing.xl}px ${ds.spacing.lg}px;text-align:center;">
  <p style="margin:0 0 ${ds.spacing.sm}px;font-family:${df};font-size:56px;font-weight:700;line-height:0.6;color:${accentColor};">"</p>
  <p style="margin:0 auto ${ds.spacing.md}px;max-width:480px;font-family:${df};font-size:23px;font-weight:400;font-style:italic;line-height:1.6;color:#ffffff;">${quote}</p>
  <p style="margin:0 0 ${ds.spacing.xs}px;font-family:${bf};font-size:10px;font-weight:600;letter-spacing:4px;text-transform:uppercase;color:rgba(255,255,255,0.45);">— ${attribution}, Verified Customer</p>
  <p style="margin:0;font-size:17px;color:${accentColor};letter-spacing:3px;">★ ★ ★ ★ ★</p>
</td></tr>`
}
...
<tr><td bgcolor="${accentColor}" style="padding:${ds.spacing.xl}px ${ds.spacing.lg}px;text-align:center;">
  <h2 style="margin:0 0 ${ds.spacing.md}px;font-family:${df};font-size:${ds.typography.h2}px;font-weight:700;line-height:1.2;letter-spacing:-0.3px;color:${accentTextColor};">${copy.cta_headline || ''}</h2>
  ${hasDiscount ? `<p style="margin:0 0 ${ds.spacing.sm}px;font-family:${df};font-size:24px;font-weight:400;letter-spacing:8px;color:${accentTextColor};">${offerUpper}</p>` : ''}
  <a href="#" style="display:inline-block;background:transparent;border:1px solid ${accentTextColor};color:${accentTextColor};font-family:${bf};font-size:11px;font-weight:600;letter-spacing:5px;text-transform:uppercase;text-decoration:none;padding:${bpv}px ${bph}px;border-radius:${br};">${copy.cta_button || 'SHOP NOW'}</a>
  ${urgency ? `<p style="margin:${ds.spacing.sm}px 0 0;font-family:${bf};font-size:12px;font-weight:400;letter-spacing:1px;color:${accentTextColor};opacity:0.7;">${urgency}</p>` : ''}
</td></tr>`
}
...
<tr><td bgcolor="${accentColor}" style="padding:${ds.spacing.xl}px ${ds.spacing.lg}px;text-align:center;">
  ...
  ${hasDiscount ? `<div style="display:inline-block;background:rgba(255,255,255,0.2);border:2px dashed rgba(255,255,255,0.5);padding:14px 44px;margin:0 0 ${ds.spacing.md}px;"><span style="font-family:${df};font-size:28px;font-weight:700;letter-spacing:6px;color:${accentTextColor};">${offerUpper}</span></div><br>` : ''}
...
<tr><td bgcolor="#faf9f7" style="padding:${ds.spacing.xl}px ${ds.spacing.lg}px;text-align:center;border-top:1px solid #e8e4de;">
  <h2 style="margin:0 0 ${ds.spacing.md}px;font-family:${df};font-size:${ds.typography.h2}px;font-weight:700;line-height:1.2;color:#1a1a1a;">${copy.cta_headline || ''}</h2>
  <a href="#" style="display:inline-block;background:transparent;border:1px solid #1a1a1a;color:#1a1a1a;font-family:${bf};font-size:11px;font-weight:600;letter-spacing:5px;text-transform:uppercase;text-decoration:none;padding:${bpv}px ${bph}px;border-radius:${br};">${copy.cta_button || 'SHOP NOW'}</a>
  ${urgency ? `<p style="margin:${ds.spacing.sm}px 0 0;font-family:${bf};font-size:12px;color:#9a8c7e;">${urgency}</p>` : ''}
</td></tr>`
}
...
export function ctaBlock_dark({ copy, offer, isWelcome, isDiscountEmail, accentColor, accentTextColor, primaryColor, df, bf, ds }) {
  ...
  return `
<tr><td bgcolor="#111111" style="padding:${ds.spacing.xl}px ${ds.spacing.lg}px;text-align:center;">
  <h2 style="margin:0 0 ${ds.spacing.md}px;font-family:${df};font-size:${ds.typography.h2}px;font-weight:700;line-height:1.2;color:#ffffff;">${copy.cta_headline || ''}</h2>
  ${hasDiscount ? `<p style="margin:0 0 ${ds.spacing.sm}px;font-family:${df};font-size:24px;font-weight:400;letter-spacing:8px;color:${accentColor};">${offerUpper}</p>` : ''}
  <a href="#" style="display:inline-block;background:${accentColor};color:${accentTextColor};font-family:${bf};font-size:11px;font-weight:600;letter-spacing:5px;text-transform:uppercase;text-decoration:none;padding:${bpv}px ${bph}px;border-radius:${br};">${copy.cta_button || 'SHOP NOW'}</a>
  ${urgency ? `<p style="margin:${ds.spacing.sm}px 0 0;font-family:${bf};font-size:12px;font-weight:400;letter-spacing:1px;color:rgba(255,255,255,0.5);">${urgency}</p>` : ''}
</td></tr>`
}
...
export function footerBlock({ brandName, logoUrl, tagline, df, bf }) {
  const logoContent = logoUrl
    ...
    : `<p style="margin:0 0 8px;font-family:${df};font-size:11px;font-weight:400;letter-spacing:7px;text-transform:uppercase;color:rgba(255,255,255,0.6);">${brandName.toUpperCase()}</p>`
  return `
<tr><td bgcolor="#111111" style="padding:44px 48px;text-align:center;">
  ${logoContent}
  ${tagline ? `<p style="margin:0 0 16px;font-family:${bf};font-size:12px;font-weight:400;letter-spacing:1px;color:rgba(255,255,255,0.35);">${tagline}</p>` : ''}
  <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%">
...
export function assembleEmail({
  ...
  // Use brand's actual site fonts — this is the key change in v2.2
